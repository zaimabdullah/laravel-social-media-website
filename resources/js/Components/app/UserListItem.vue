...
<template>
  <div class="cursor-pointer bg-white transition-all border-2 border-transparent hover:border-indigo-500">
    ...
    <img :src="user.avatar_url || '/img/default_avatar.webp'" class="w-[32px] rounded-full" alt="" />
    <div class="flex justify-between flex-1">
      <h3 class="font-black">{{ user.name }}</h3>
      <div class="flex gap-1">
        <button class="text-xs py-1 px-2 rounded bg-emerald-500 hover:bg-emerald-600 text-white" v-if="forApprove"
